...
- **✅ 2026-08-08 起不再静默**：signal_monitor 硬校验 `rules` 为空/缺失 → stderr 报 `ERROR ... 缺少 rules 数组` + exit 2（包装脚本 grep ERROR/WARNING 会推微信）。干 plan（Web 面板格式 `entry.trigger_price`+`monitor.*`）见报错即按标准格式重建。另：该修复用 `sys.exit()`，`run_directory_once`/`main --loop` 两处循环已加 `except SystemExit` 隔离（单个坏计划不拖垮其他计划）——给引擎新增"终止类"异常时记得同步检查所有循环调用方的异常捕获（SystemExit/KeyboardInterrupt 不被 `except Exception` 捕获）。
...
**破位空"可建 vs 拒"的判别 = RSI 超卖与否（AAVE 08-04 拒 vs 08-08 建 对照）**：
...
另：扫描器 `stability.warning`（名单跳变 / jaccard 偏低，本轮 0.385）说明短线资金切换快 → 缩短 plan `expires_at` 并在输出中提示。08-08 实例：jaccard 0.286（更跳）→ expires 定为 **12h**（24h 减半），两新计划均按此执行。

**评分取向:高分=正在突破/刚突破,回踩结构分数天然低(2026-08-06,用户问"为什么回踩机会分数低"时解释用)**:
- 突破加分与回踩加分**互斥**(同一时刻只算一个);突破时价格站上所有短均线→基础分全拿+突破加分=70-86;回踩时跌破短均线→丢 1H/15m 站上 MA7 基础分,只剩回踩加分=50-60
- 空头侧**没有**回踩加分项→反弹空分数必然 <50
- 结论:按分数选候选=自动过滤掉健康回踩机会;要抓回踩须主动筛 46-60 分区间"突破后回踩 MA25 缩量企稳"结构,不能只盯高分榜(流程层未定,2026-08-06 探讨中)
...
**二次修复 (2026-08-08, XRP 事故, commit 3c2ce12)**:原处理**无条件删 plan**。若 plan 过期时**持仓还在**,删 plan 后 `manage_all_positions()` 按 plan 文件遍历就找不到该币 → TP1 成交后移保本**永不执行**(SL 停留在原止损位 + 全量数量)。XRP 实例:08-07 22:00 plan 过期删除 → 08-08 01:00 TP1 成交 → SL 96.2@1.0523 未移保本、未减量(algo 挂单 `updateTime==createTime` 证实从未被动过)。**修复**:PLAN_EXPIRED 分支先查持仓,有 → **保留 plan 文件**(与 INVALIDATION 分支一致),无 → 照旧删。**语义:过期 = 停止入场监控**(signal_monitor 过期后 return,不评估规则、不写 TRIGGER),但**持仓管理(移保本/通知)持续到平仓**,平仓后 detect_position_changes 照旧删 plan,无僵尸。**已知副作用**:plan 保留期间 signal_monitor 无防重复、每分钟重复写 PLAN_EXPIRED 事件,executor 每分钟处理(查持仓→保留→删事件),无通知无交易影响,可接受。**判定移保本是否执行过的铁证**:algo 挂单 `updateTime > createTime` 或 SL 数量 == 剩余持仓 = 执行过;`updateTime == createTime` + SL 数量仍为全量 = 从未被动过。
