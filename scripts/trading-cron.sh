...
EVENT_RESULT=$(python3 "$EXECUTOR" process-events 2>&1 | grep -v '^\[' | grep -v 'INFO\|WARNING\|ERROR\|运行模式')
if [ "$EVENT_RESULT" != "[]" ] && [ -n "$EVENT_RESULT" ]; then
    OUTPUT="📋 事件处理:\n$EVENT_RESULT\n"
fi

# 2. 管理持仓（TP1平半仓+移保本）
MANAGE_RESULT=$(python3 "$EXECUTOR" manage 2>&1 | grep -v '^\[' | grep -v 'INFO\|WARNING\|ERROR\|运行模式')
